...
def call_warning_interface(ip, port):
    """
    调用指定 IP 和端口的 /warning 接口
    """
    url = f"http://{ip}:{port}/warning"
    try:
        response = requests.get(url, timeout=1)
        print(f"调用 {url} 的结果：{response.status_code}")
    except Exception as e:
        print(f"调用 {url} 时发生错误：{e}")

def start_threads(ip_port_list):
    """
    启动多个线程分别调用 IP 地址和端口的 /warning 接口
    :param ip_port_list: 包含多组 IP 地址和端口的列表，每组是一个元组 (ip, port)
    """
    threads = []
    for ip, port in ip_port_list:
        thread = threading.Thread(target=call_warning_interface, args=(ip, port), daemon=True)
...
def send():
    ip_port_list = []
    with open('/home/sunrise/Desktop/yolo_workspace/Web_RDK_Performance_Node-main/ip.txt', 'r') as f:
        pass
        res = f.readlines()
        res = [r.replace('\n', '') for r in res]
        ip_port_list = [(i.split()[0], i.split()[1]) for i in res]
